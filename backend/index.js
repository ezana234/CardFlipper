        ...
        if (!errors.isEmpty()) {
            ...
            try {
                ...
                chatNsp.on("connection", (socket) => {
                    //console.log("Num of Clients in ChatNSP : ", chatNsp.adapter.sids.size)
                    // Check if the user is in the room if the game is active.
...
app.post("/sendChat", Utils.authenticateJWT, validateMessage, (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        Utils.decodeToken(req).then(async (userObj) => {
            ...
            const chatNsp = io.of("/chat/" + roomID)
            chatNsp.emit("text", roomService.messageService(userObj.username, roomID, message))
